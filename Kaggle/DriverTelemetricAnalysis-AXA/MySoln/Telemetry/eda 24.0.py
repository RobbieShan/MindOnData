...
#                                                ,'PC5'
    ...
    minDist = db.transform(Xpca).min(axis=1) # G    db = DBSCAN(eps=0.7).fit(Xpca)et  distance for each point from the nearest cluster
    zMinDist = (1-sp.stats.norm.cdf(sp.stats.mstats.zscore(minDist)))
    
#    plt.figure()
#    plt.hist(zMinDist,range=(0,1))
...
#    print "##############################DBSCAN  X Below#################################################"
    ...
    return zMinDist ##KMeans ZScores
